...
impl Daemon {
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        // Create job source (Stratum V1, Stratum V2, or Dummy)
        ...
        if let Ok(pool_url) = env::var("MUJINA_POOL_URL") {
            ...
            match endpoint.protocol {
                PoolProtocol::StratumV2 => {
                    // PoolEndpoint::parse only produces StratumV2 alongside a
                    // key, so this reports a broken parser rather than bad
                    // input.
                    let authority_pubkey = endpoint
                        .authority_pubkey()
                        .context("Stratum V2 endpoint parsed without an authority public key")?;
