...
pub enum SourceEvent {
    ...
    ///
    /// The payload is the number of shares accepted, derived from the
    /// sequence number range in `SubmitSharesSuccess`. SV1 has no per-batch
    /// ACK, so this variant is only emitted by SV2 sources.
    SharesAccepted(u32),
